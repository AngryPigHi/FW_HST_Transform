using System;
using System.Collections.Generic;

#nullable disable

namespace OracleDBHelperServices.Models
{
    public partial class HNydQlr
    {
        public string Bgbh { get; set; }
        public string Bz { get; set; }
        public string DjdcbIndex { get; set; }
        public string Djh { get; set; }
        public string Dlrdhhm { get; set; }
        public string Dlrsfzms { get; set; }
        public string Dlrxm { get; set; }
        public string Dlrzjh { get; set; }
        public string Dlrzjlx { get; set; }
        public string Dwxz { get; set; }
        public decimal? Dymj { get; set; }
        public string Frdbdhhm { get; set; }
        public byte[] Frdbsfzms { get; set; }
        public string Frdbxm { get; set; }
        public string Frdbzjh { get; set; }
        public string Frdbzjlx { get; set; }
        public decimal? Ftmj { get; set; }
        public DateTime? Gxrq { get; set; }
        public DateTime? Jlrq { get; set; }
        public string Mjdw { get; set; }
        public string NydQlrIndex { get; set; }
        public string Qlrlx { get; set; }
        public string Qlrmc { get; set; }
        public string Qlrzjh { get; set; }
        public string Qlrzjlx { get; set; }
        public string Qsdwdm { get; set; }
        public string Sfldsyqr { get; set; }
        public string Sflmsuqr { get; set; }
        public string Sflmsyqr { get; set; }
        public string Sjzgbm { get; set; }
        public decimal? Symj { get; set; }
        public string Txdz { get; set; }
        public int? Xh { get; set; }
        public string Yzbm { get; set; }
    }
}
