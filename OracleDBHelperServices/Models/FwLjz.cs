using System;
using System.Collections.Generic;

#nullable disable

namespace OracleDBHelperServices.Models
{
    public partial class FwLjz
    {
        public string B { get; set; }
        public string Bdcdyfwlx { get; set; }
        public string Bdcdyh { get; set; }
        public string Bdczt { get; set; }
        public string Bgbh { get; set; }
        public string Bz { get; set; }
        public string Cb { get; set; }
        public string Cqly { get; set; }
        public string D { get; set; }
        public DateTime? Dcsj { get; set; }
        public string Dcyj { get; set; }
        public string Dcz { get; set; }
        public string Dh { get; set; }
        public byte? Dscs { get; set; }
        public byte? Dxcs { get; set; }
        public long? Fdckfqybh { get; set; }
        public string Fjsm { get; set; }
        public string FwDcbIndex { get; set; }
        public string FwKfxmIndex { get; set; }
        public string FwXmxxIndex { get; set; }
        public byte? Fwcs { get; set; }
        public string Fwjg { get; set; }
        public string Fwjg2 { get; set; }
        public string Fwjg3 { get; set; }
        public string Fwmc { get; set; }
        public string Fwyt { get; set; }
        public string Fwyt2 { get; set; }
        public string Fwyt3 { get; set; }
        public string Fwzt { get; set; }
        public DateTime? Gxrq { get; set; }
        public string Gyqk { get; set; }
        public DateTime? Jgrq { get; set; }
        public string Jsgcghxkzid { get; set; }
        public string Jsydghxkzid { get; set; }
        public string Jsydpzsid { get; set; }
        public string Jsydsyqid { get; set; }
        public string Jzgcsgxkzid { get; set; }
        public string Jzwzt { get; set; }
        public string Ljzh { get; set; }
        public string Lszd { get; set; }
        public string Mph { get; set; }
        public string N { get; set; }
        public DateTime? Qsrq { get; set; }
        public string Qsxz { get; set; }
        public DateTime? Sbjgjdjssj { get; set; }
        public DateTime? Sbjgjdqssj { get; set; }
        public decimal? Scdxmj { get; set; }
        public decimal? Scjzmj { get; set; }
        public decimal? Scqtmj { get; set; }
        public string Tdyt { get; set; }
        public string X { get; set; }
        public string Xkid { get; set; }
        public string Xmid { get; set; }
        public string Xmmc { get; set; }
        public decimal? Ycdxmj { get; set; }
        public decimal? Ycjzmj { get; set; }
        public decimal? Ycqtmj { get; set; }
        public string Ysdm { get; set; }
        public string Zldz { get; set; }
        public string Zrzh { get; set; }
        public string Zsdwqyid { get; set; }
        public DateTime? Ztrq { get; set; }
        public int? Zts { get; set; }
        public decimal? Zydmj { get; set; }
        public decimal? Zzdmj { get; set; }
        public DateTime? Zzrq { get; set; }
        public string Qlrmc { get; set; }
        public string Qlrzjh { get; set; }
        public string Sjhj { get; set; }
        public string DataSorce { get; set; }
        public string DataState { get; set; }
        public string Qq { get; set; }
        public string TaicangChcghYc { get; set; }
        public string TaicangChcghSc { get; set; }
        public string TaicangKfsName { get; set; }
        public string TaicangKfsNo { get; set; }
        public string Fdckfsbh { get; set; }
        public string Yccgbh { get; set; }
        public string Sccgbh { get; set; }
        public string Sgbh { get; set; }
        public string Gabh { get; set; }
        public string Qyhf { get; set; }
        public string Lszdxg { get; set; }
        public string Xgry { get; set; }
        public string Xgbz { get; set; }
        public string Xgzt { get; set; }
        public string Xgrq { get; set; }
        public string Hscount { get; set; }
        public string Checkdate { get; set; }
        public string DahXlf { get; set; }
    }
}
