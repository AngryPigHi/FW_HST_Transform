using System;
using System.Collections.Generic;

#nullable disable

namespace OracleDBHelperServices.Models
{
    public partial class ZrzyK320585
    {
        public decimal Objectid { get; set; }
        public string Ysdm { get; set; }
        public int? Bsm { get; set; }
        public string Zrzydyh { get; set; }
        public string Syqlx { get; set; }
        public string Zrzytzm { get; set; }
        public string Zl { get; set; }
        public decimal? Zmj { get; set; }
        public string Mjdw { get; set; }
        public string Zldwdm { get; set; }
        public string Zldwmc { get; set; }
        public string Qsdwdm { get; set; }
        public string Qsdwmc { get; set; }
        public string Zrszd { get; set; }
        public string Zrszn { get; set; }
        public string Zrszx { get; set; }
        public string Zrszb { get; set; }
        public string Bhqk { get; set; }
        public string Syqr { get; set; }
        public short? Zsl { get; set; }
        public string Ytgzyq { get; set; }
        public string Tsbhyq { get; set; }
        public string Sthxyq { get; set; }
        public string Tfh { get; set; }
        public string Bz { get; set; }
        public string Bgbh { get; set; }
        public string Fldm { get; set; }
        public string Ssqy { get; set; }
        public string Sm { get; set; }
        public string Kczydjdyh { get; set; }
    }
}
