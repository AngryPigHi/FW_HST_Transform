using System;
using System.Collections.Generic;

#nullable disable

namespace OracleDBHelperServices.Models
{
    public partial class HZhDjdcb
    {
        public string Bdcdyh { get; set; }
        public string Bdcdyhzt { get; set; }
        public string Bgbh { get; set; }
        public string Bz { get; set; }
        public string Db { get; set; }
        public DateTime? Gxrq { get; set; }
        public string Gzwlx { get; set; }
        public string Hddm { get; set; }
        public string Hdmc { get; set; }
        public string Hdwz { get; set; }
        public string Hdyt { get; set; }
        public string Hyglbh { get; set; }
        public string Jlbcldw { get; set; }
        public string Jlbclr { get; set; }
        public DateTime? Jlbclrq { get; set; }
        public string Jlbsdjz { get; set; }
        public string Jlbshr { get; set; }
        public DateTime? Jlrq { get; set; }
        public string Kzbh { get; set; }
        public string Pzwh { get; set; }
        public DateTime? Qsrq { get; set; }
        public string Qsxz { get; set; }
        public string Qtsm { get; set; }
        public string Syjbzyj { get; set; }
        public string Syjjnqk { get; set; }
        public string Xmmc { get; set; }
        public string Xmxz { get; set; }
        public string Ydfw { get; set; }
        public decimal? Ydmj { get; set; }
        public string Yhlxa { get; set; }
        public string Yhlxb { get; set; }
        public string Yhspdw { get; set; }
        public string Yhwzsm { get; set; }
        public decimal? Yhzmj { get; set; }
        public string ZhDjdcbIndex { get; set; }
        public string ZhXmxxIndex { get; set; }
        public decimal? Zhax { get; set; }
        public string Zhdm { get; set; }
        public decimal? Zhmj { get; set; }
        public string Zhtzm { get; set; }
        public DateTime? Zzrq { get; set; }
        public string Qlrmc { get; set; }
        public string Qlrzjh { get; set; }
    }
}
